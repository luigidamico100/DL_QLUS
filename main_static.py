...
# Models to choose from [resnet, efficientnet-b0, alexnet, vgg, squeezenet, densenet, inception]
# Mode to choose from [random_frame_from_clip, entire_clip, entire_clip_1ch]
model_name = 'efficientnet-b0'
num_classes = 3
num_epochs = 5 if on_cuda else 3
...
model_ft, input_size = stat_mod_ut.initialize_model(model_name, num_classes, feature_extract, use_pretrained=True)
print(model_ft)
...
train_dl, val_dl, test_dl, train_ds, val_ds, test_ds = dataload_ut.get_mat_dataloaders(dataload_ut.all_classes, basePath=DATASET_PATH, num_workers=num_workers, 
                                                                                       batch_size=batch_size, mode=mode, replicate_all_classes=10,
                                                                                       target_value=False)
dataloaders_dict = {
    'train' : train_dl, 
    'val' : val_dl
...
model, hist = stat_mod_ut.train_model(model_ft, dataloaders_dict, criterion, optimizer_ft, num_epochs=num_epochs, is_inception=(model_name=="inception"))

#%%


stat_mod_ut.plot_and_save(model, hist)
