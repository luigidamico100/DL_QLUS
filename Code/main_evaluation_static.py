...
if __name__ == '__main__':
    num_batches = 1000000 if on_cuda else 2
    ...
    if not experiment_all_fold:
        ...
        _, train_score, train_metric = stat_mod_ut.eval_model(model_evaluation, dataloaders_dict['train'], loss, metric, num_batches=num_batches)
        _, val_score, val_metric= stat_mod_ut.eval_model(model_evaluation, dataloaders_dict['val'], loss, metric, num_batches=num_batches)
        _, test_score, test_metric= stat_mod_ut.eval_model(model_evaluation, dataloaders_dict['test'], loss, metric, num_batches=num_batches)
        ...
        for fold_test in fold_test_list:
            ...
            if classification:
                dataloaders_dict, _ = dataload_ut.get_mat_dataloaders_v2(classification_classes, basePath=DATASET_PATH, num_workers=num_workers, fold_test=fold_test,
                                                                                           ...
                                                                                           target_value=True,
                                                                                           train_samples=False, val_samples=False, test_samples=True)
                test_spearmanCorr = stat_mod_ut.eval_spearmanCorr(model_evaluation, dataloaders_dict['test'], num_batches=num_batches)
                print('n_samples: {}, test_metric: {:.2f}, SpearmanCorr: {:.2f}\n'.format(n_samples, test_metric, test_spearmanCorr))
            running_metrics = np.concatenate((running_metrics, np.array([[n_samples, test_metric, test_spearmanCorr]])))
        ...
        spearmanCorr_weighted = (running_metrics[:,2] * metrics_weights).sum()
        print('------------------- Results------------------- ')
        print('Test ' + str(metric)+ ' for each fold: ')
        print(running_metrics)
        print('\nAggregate ' + str(metric) + ': '+str(metric_weighted) +'\t Aggregate spearmanCorr: '+str(spearmanCorr_weighted))
